...
import * as bootstrap from "bootstrap"; // 🔥 FALTABA ESTO

const CrudReactivos = ({ user }) => {
  ...
  const columns = [
    ...
    },
    {
      name: "Stand / Col / Fila",
      selector: (row) =>
        `${row.stand || "-"} / ${row.columna || "-"} / ${row.fila || "-"}`,
      sortable: false,
      width: "180px",
    },
    {
  ...
  const cargarReactivos = async () => {
    try {
      const res = await apiAxios.get("/api/reactivos"); // 🔥 CORREGIDO
  ...
  const toggleEstado = async (id, estadoActual) => {
    ...
    try {
      await apiAxios.put(`/api/reactivos/estado/${id}`, { // 🔥 CORREGIDO
        estado: nuevoEstado,
      });
  ...
  const filtered = reactivos.filter(
    (item) =>
      ...
      String(item.nom_reactivo || "")
        .toLowerCase()
        .includes(filterText.toLowerCase()) ||
      String(item.nom_reactivo_ingles || "")
        .toLowerCase()
        .includes(filterText.toLowerCase())
  );
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <ReactivoForm
                ...
                hideModal={() => {
                  const modal =
                    document.getElementById("modalReactivo");
                  const bsModal =
