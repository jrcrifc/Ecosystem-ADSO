...
export default function CrudEquipo() {
  ...
  const cambiarEstado = async (equipo) => {
    ...
    const result = await Swal.fire({
      title: "¿Cambiar estado?", text: `El equipo pasará a ${nuevoEstado === 1 ? "ACTIVO" : "INACTIVO"}`,
    ...
    try {
      ...
      Swal.fire({ icon: "success", title: nuevoEstado === 1 ? "Activado" : "Inactivado", timer: 1500, showConfirmButton: false });
      getAllEquipos();
    } catch (error) {
  ...
  const columns = [
    ...
    { name: "Cuentadante", selector: (row) => row.id_usuario_cuentadante || "-", width: "120px" },
    {
      name: "Foto", width: "120px", center: true,
      cell: (row) => (
        <div style={{ padding: "5px", cursor: row.foto_equipo ? "pointer" : "default" }}>
          {row.foto_equipo ? (
            <img
              ...
              style={{ width: "80px", height: "80px", objectFit: "cover", borderRadius: "8px", border: "2px solid #28a745", transition: "transform 0.2s", boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }}
    ...
    {
      name: "Estado", center: true, width: "120px",
      cell: (row) => (
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="d-flex gap-2 justify-content-center">
          <button className="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#modalEquipo" onClick={() => setSelectedEquipo(row)} title="Editar equipo">
            <i className="fas fa-edit"></i>
          </button>
          <button className={`btn btn-sm ${row.estado === 1 ? "btn-outline-danger" : "btn-outline-success"}`} onClick={() => cambiarEstado(row)} title={row.estado === 1 ? "Inactivar" : "Activar"}>
    ...
    <div className="container mt-4">
      ...
      <div className="row mb-4 align-items-center">
        <div className="col-md-6">
          <input type="text" className="form-control" placeholder="Buscar por nombre, grupo, marca, placa o cuentadante..." value={filterText} onChange={(e) => setFilterText(e.target.value)} />
        </div>
      ...
      <DataTable columns={columns} data={filteredEquipos} pagination paginationPerPage={10} highlightOnHover striped responsive noDataComponent="No hay equipos registrados" />
        ...
        <div className="modal-dialog modal-lg">
          <div className="modal-content">
            <div className="modal-header bg-primary text-white">
              <h5 className="modal-title" id="modalEquipoLabel">{selectedEquipo ? "Editar Equipo" : "Registrar Nuevo Equipo"}</h5>
              <button type="button" className="btn-close btn-close-white" data-bs-dismiss="modal" onClick={() => hideModal("modalEquipo")} aria-label="Close"></button>
            </div>
            <div className="modal-body">
              <EquipoForm selectedEquipo={selectedEquipo} refreshParent={getAllEquipos} hideModal={() => hideModal("modalEquipo")} />
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="modal-dialog modal-xl modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              ...
              <button type="button" className="btn-close" data-bs-dismiss="modal" onClick={() => hideModal("largePhotoModal")} aria-label="Close"></button>
            </div>
            <div className="modal-body text-center p-4">
              {largePhoto && (
                <img
                  ...
                  style={{ maxWidth: "100%", maxHeight: "80vh", objectFit: "contain", borderRadius: "12px", boxShadow: "0 10px 30px rgba(0,0,0,0.3)" }}
                  onError={(e) => { e.target.src = "/img/no-image.png"; }}
                />
              )}
            </div>
            <div className="modal-footer">
              <button type="button" className="btn btn-secondary" data-bs-dismiss="modal" onClick={() => hideModal("largePhotoModal")}>Cerrar</button>
            </div>
          </div>
        </div>
      </div>
