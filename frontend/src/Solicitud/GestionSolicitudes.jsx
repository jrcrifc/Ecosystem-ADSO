...
const GestionSolicitudes = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "120px",
      cell: (row) => {
        const rol = (row.usuario?.rol || "").toLowerCase();
        if (rol === "aprendiz") {
          return <span style={{ color: "#d97706", fontWeight: "700", fontSize: "11px" }}>⚡ Hoy (Mismo día)</span>;
        }
        if (rol === "instructor") {
          return <span style={{ color: "#0369a1", fontWeight: "700", fontSize: "11px" }}>📅 3-5 Días hábiles</span>;
        }
        return <span className="text-muted small">{row.usuario?.rol || "-"}</span>;
      }
    },
    {
      name: "Estado Actual",
